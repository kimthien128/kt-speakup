...
from .auth import UserInDB, get_auth_service
...
@router.post("")
async def add_vocab(request: Request, current_user: dict = Depends(get_auth_service), vocab_service: VocabService = Depends(get_vocab_service)):
    try:
...
@router.get("/{chat_id}")
async def get_vocab(chat_id: str, current_user: UserInDB = Depends(get_auth_service), vocab_service: VocabService = Depends(get_vocab_service)):
    try:
...
async def delete_vocab(chatId: str, vocab_id: str, current_user: UserInDB = Depends(get_auth_service), vocab_service: VocabService = Depends(get_vocab_service)):
    try:
